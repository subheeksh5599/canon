...
[![▶ Watch the demo — 2:24, real screen capture of the live venue](docs/media/canon-demo-poster.png)](https://youtu.be/DnEUdOQSX1k)

**[▶ Watch the demo (2:24)](https://youtu.be/DnEUdOQSX1k)** &nbsp;·&nbsp; **[ Live venue ↗ ](https://canon-venue.vercel.app)** &nbsp;·&nbsp; **[ Console ↗ ](https://canon-venue.vercel.app/console)** &nbsp;·&nbsp; **[ Local copy ↗ ](docs/media/canon-demo.mp4)**

[![Watch on YouTube](https://img.shields.io/badge/▶%20Watch%20the%20demo-YouTube-FF0000?labelColor=1f1f23)](https://youtu.be/DnEUdOQSX1k)

_Click the frame to play it. The real venue driven live in one take — no animation, no mock, nothing staged._ A $16 deal is quoted from doctrine stored in memory, escrow locks in real USDC on Base Sepolia (the Basescan hash is opened on screen and the amount read off the explorer), the deletion gate is run from the console and the venue **refuses to rule** once the memory is gone, an accepted bonded appeal moves doctrine v1 → v2 for the whole venue, and the registered Virtuals agent's settlement is opened on-chain. Every hash in the recording reproduces with `scripts/verify_receipts.py`.
...
### Fourth run — the agent's own inference delivers the job (Sep 10)

The registered Virtuals agent produced the work itself. On a real $3 venue job under doctrine v2 ($1.20 upfront / $1.80 escrow / $0.30 bond, terms digest `0x3a82ac89…`), the deliverable was generated by the agent's **Virtuals-hosted compute endpoint** — a real completion (`generation gen-1789021529-lQTOAU4WPbwdOTiaF2KV`, model `deepseek/deepseek-chat`, 163 tokens, inference cost $0.000114, billed against the Spark-Tier credits), recorded against the job. The venue then settled it.

| Step | Contract | Transaction hash |
|---|---|---|
| Register $3 deal, provider = Virtuals agent | #16 | [`0x3313ab46…ef81`](https://sepolia.basescan.org/tx/0x3313ab467febe3a3d13b9a8e43168b7f9e5373f1c57e378568ba00ac1a91ef81) |
| Escrow lock $1.80 + $0.30 bond | | [`0x3ac590b0…1758`](https://sepolia.basescan.org/tx/0x3ac590b0e0422e452c8005f28a9d6cb929abf35a62c0b256c291e2b7cc2e1758) |
| Delivered — escrow paid to the agent | | [`0xf428933a…0dcd`](https://sepolia.basescan.org/tx/0xf428933a83ec8ce17177ece0e0bd390cd8f9ddc1c48027a50068f30f9f910dcd) |
...
| Virtuals partner stack | ✅ FULLY EXERCISED — a real agent registered on Virtuals EconomyOS ("Canon Venue Provider", `01a08682-…`, wallet `0x1776e…`) is admitted as counterparty Prov-03, **produced its deliverable on its own Virtuals-hosted compute** (generation `gen-1789021529-lQTOAU4WPbwdOTiaF2KV`, model `deepseek/deepseek-chat`, real inference cost $0.000114, Spark-Tier credits) and **received the USDC settlement** for it. Fourth-run receipt below |
